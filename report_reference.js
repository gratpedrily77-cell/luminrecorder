...
//  1. TIPS 提示词对象 (25行)
//  2. 工具函数 (75行)
//     tipIcon, formatShort, parseMin, fmtMin, fmtHrs,
//     calcStats, fmtCV, fmtSD, sessionClock, devClass, devStr,
//     escHtmlApp, getActColor, getDay, truncateActPath,
//     STACKED_PALETTE, getStackedColor, hexRgba
//  3. computeRange — 汇总数据 (38行)
//  4. Chart 工具 gridCfg, mkChart (14行)
//  5. renderCalendar — 日历视图 (61行)
//  6. renderDayOverview — 日览 (156行)
//  7. renderWeekOverview — 周览 (259行)
//     含: 统一汇总卡片(有效天数+6指标各含日均+CV)
//     含: 填充折线图(三维时间)
//     含: 每日汇总表(σ/CV行)
//     含: 任务记录表(操作列)
//     含: 专注时段明细表(操作列)
//  8. renderMonthOverview — 月览 (261行)
//     同上结构
//  9. renderSleep — 作息图表 (244行)
//     含: mod-24滚动Y轴
//     含: 起床/睡觉散点折线
//     含: 作息明细表
// 10. 时段分析 — renderSessAnalysis (189行)
//     含: 类别筛选下拉框
//     含: 筛选时只显示有数据的天(连续折线)
//     含: CV卡片(仅选中类别时显示)
//     含: 类别汇总明细表(含CV列)
// 11. 任务分析 — renderTaskAnalysis (343行)
//     含: 层级切换(一级/二级/三级)
//     含: 类别筛选下拉框
//     含: 每日时长趋势折线图
//     含: 类别饼图
//     含: 每日任务数量(柱+线双轴)
//     含: 效率趋势图(固定三级, 线性/对数/归一化, Y轴手动, 类别筛选)
//     含: 类别汇总明细表(三级时含CV/数量/效率/效率CV)
// 12. 堆积图 — renderStackedArea (340行)
//     含: 绝对值+百分比两张图
//     含: 动态归一化(隐藏类别后重算百分比)
//     含: 清醒虚线参考线
//     含: 统计卡片(清醒/任务/特殊/休息/分心/空闲)
// 13. state 对象参考
//
// 总计: ~2157行原始代码
// ============================================================
...
const TIPS = {
  ...
  util: '📊 时间利用率\n实际专注占可支配时长的比例，衡量真正可用于学习的时间中有多少真正专注。\n\n≥50% 合格\n\n公式：实际专注 ÷ 可支配时长 × 100%\n可支配时长 = 清醒时长 − 特殊时段总时长\n特殊时段指吃饭、通勤、外出、休息等不可能专注的时段（type=special）\n无特殊时段时，可支配时长 = 清醒时长，与旧算法完全一致',
  deviation: '📉 偏差率（实际 vs 名义）\n实际专注与名义时长的差值比，反映真实专注量 vs 计划目标的差距。\n\n正值 = 超额完成计划\n负值 = 未达计划（走神多或提前结束）\n\n公式：(实际专注 − 名义时长) ÷ 名义时长 × 100%',
  clockDev: '⚡ 时钟偏差（时钟 vs 名义）\n时钟时长与名义时长的差值比，反映实际在场时间 vs 计划时间的偏差。\n\n正值 = 在场时间超过计划\n负值 = 比计划提前结束\n\n公式：(时钟时长 − 名义时长) ÷ 名义时长 × 100%',
  ...
  stackSpecial: '🔸 特殊时段总时长\n所选时间范围内所有标记为「特殊时段」的 session 时长总和。\n包括吃饭、通勤、外出等不可能专注的时段，按名称分组堆叠。\n\n公式：∑ (type=special 的 session 的时钟时长)',
...
function computeRange(dateStrs) {
  ...
  totals.effectiveClockMin = Math.max(0, totals.clockMin - totals.restMin);
  totals.distractMin = Math.max(0, totals.clockMin - totals.actualMin - totals.restMin);
  totals.clockVsNominal = totals.nominalMin > 0 ? Math.round((totals.clockMin - totals.nominalMin) / totals.nominalMin * 100) : null;
...
function renderDayOverview() {
  ...
  document.getElementById('tab-day').innerHTML = `
    ...
    <div class="mini-grid">
      ...
      <div class="mini-card"><div class="lbl">可支配时长</div><div class="val" style="color:var(--clock)">${s.disposableMin != null ? fmtMin(s.disposableMin) : '-'}</div><div class="sub">清醒${s.specialMin ? ' − 特殊' + fmtMin(s.specialMin) : ' (无特殊时段)'}</div></div>
    ...
    const isSpec = sess.type === 'special';
    const actual = Number(sess.actualMinutes) || 0;
    const rest = Number(sess.restMinutes) || 0;
    const distract = isSpec ? 0 : Math.max(0, cl - actual - rest);
    const eff = (!isSpec && (cl - rest) > 0) ? Math.round(actual / (cl - rest) * 100) : null;
    return `<tr${isSpec ? ' style="background:rgba(206,147,216,.06)"' : ''}>
                  <td class="fw-mono c-muted">${i + 1}${isSpec ? '<br><span style="font-size:9px;background:rgba(206,147,216,.2);color:#ce93d8;padding:1px 4px;border-radius:3px">特殊</span>' : ''}</td>
                  <td class="fw-mono">${isSpec && sess.name ? `<span style="color:#ce93d8;font-weight:600">${escHtmlApp(sess.name)}</span><br>` : ''}${sess.startTime || '-'}</td><td class="fw-mono">${sess.endTime || '-'}</td>
                  ...
                  <td class="fw-mono">${isSpec ? '-' : fmtMin(rest, true)}</td>
                  <td class="fw-mono">${isSpec ? '-' : fmtMin(distract, true)}</td>
...
function renderWeekOverview() {
    ...
    <div class="card entry-table-card" style="margin-top:16px">
      ...
      : `<div class="table-wrap" style="max-height:520px">
        ...
        return `<tr${isSpec ? ' style="background:rgba(206,147,216,.06)"' : ''}>
              ...
              <td>${isSpec ? '<span style="font-size:10px;background:rgba(206,147,216,.2);color:#ce93d8;padding:1px 6px;border-radius:3px">' + escHtmlApp(sess.name || '特殊') + '</span>' : '普通'}</td>
              <td class="fw-mono">${sess.startTime || '-'}</td><td class="fw-mono">${sess.endTime || '-'}</td>
              <td class="fw-mono c-clock">${fmtMin(cl, true)}</td>
              <td class="fw-mono c-nominal">${isSpec ? '-' : fmtMin(Number(sess.nominalMinutes) || 0, true)}</td>
  ...
  mkChart('weekThreeChart', {
    ...
    options: {
      ...
      plugins: { legend: { labels: { color: '#6b7a9e' } }, filler: { propagate: false },
...
// ═══════════════════════════════════════════
// 10. 时段分析 — sessAnaGetDates, renderSessAnalysis, sessAnaNav
// ═══════════════════════════════════════════
...
function renderSessAnalysis() {
  ...
  let totalSessCount = 0, totalClock = 0, totalNominal = 0, totalActual = 0, totalRest = 0, totalSpecial = 0;
  ...
  dateStrs.forEach(ds => {
    ...
    (day.sessions || []).forEach(sess => {
      ...
      const cat = sess.type === 'special' ? (sess.name || '特殊时段') : '普通专注';
      if (sess.type === 'special') totalSpecial += clk;
  ...
  const eff = (totalClock - totalRest) > 0 ? Math.round(totalActual / (totalClock - totalRest) * 100) : null;
  ...
  mkChart('sessAnaDailyChart', {
    ...
    options: {
      responsive: true,
      interaction: { mode: 'index', intersect: false },
      plugins: { legend: { position: 'bottom', labels: { color: '#6b7a9e', boxWidth: 12 } },
        tooltip: { callbacks: { label: ctx => { const v = ctx.parsed.y; if (!v) return null; return `${ctx.dataset.label}: ${fmtMin(Math.round(v * 60))}`; } } } },
      scales: { x: { ticks: { color: '#6b7a9e', maxRotation: s.mode === 'month' ? 45 : 0 }, grid: gridCfg },
...
// ═══════════════════════════════════════════
// 11. 任务分析 — taskAnaGetDates, renderTaskAnalysis, taskAnaNav
// ═══════════════════════════════════════════
